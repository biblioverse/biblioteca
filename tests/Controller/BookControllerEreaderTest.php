...
class BookControllerEreaderTest extends WebTestCase
{
    ...
    public function testSendToEreaderSuccessfully(): void
    {
        $client = static::createClient();
        $userRepository = static::getContainer()->get(UserRepository::class);
        $testUser = $userRepository->findOneBy(['username' => UserFixture::USER_USERNAME]);
        ...
        // Verify the route is accessible and the form is present
        $crawler = $client->request(Request::METHOD_GET, '/books/'.$book->getId().'/'.$book->getSlug());
        self::assertResponseIsSuccessful();

        // Find the send form
        $crawler->filter('form[action*="send-to-ereader"]')->form();
    }
