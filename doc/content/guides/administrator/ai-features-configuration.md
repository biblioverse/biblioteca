...
Biblioteca includes AI-powered commands to organize your entire library:

- **`books:ai-organize`** - Master command that runs all harmonization in order
- **`books:authors-harmonize`** - Unify author name variations
- **`books:series-harmonize`** - Detect series and clean up titles
- **`books:tags-harmonize`** - Assign standardized genres

These commands use a predefined list of 37 literary genres (available in French, English, German, Spanish) to ensure consistent categorization across your library.

```bash
# Organize everything with French genres
docker compose exec biblioteca bin/console books:ai-organize --language fr --apply
```

See the [Commands documentation](/guides/administrator/commands) for detailed usage.
